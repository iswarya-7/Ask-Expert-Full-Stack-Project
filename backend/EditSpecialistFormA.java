...
@WebServlet("/EditSpecialistFormA")
public class EditSpecialistFormA extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String phone = request.getParameter("mobile");    // Changed from phone to mobile to match form
            String category = request.getParameter("expertiseCategory"); // Changed to match form
            String expert_domain = request.getParameter("expertiseDomain"); // Changed to match form
            String experience = request.getParameter("yearsExperience"); // Changed to match form
            ...
            String getCurrentDataSql = "SELECT profile_photo, password FROM specialist_regdetails WHERE id=?";
...
//                fileName = System.currentTimeMillis() + "_" + Paths.get(filePart.getSubmittedFileName()).getFileName().toString();
            ...
            String sql = "UPDATE specialist_regdetails SET full_name=?, gender=?, dob=?, email=?, mobile=?, city=?, "
                    + "profile_photo=?, expertise_category=?, expertise_domain=?, years_experience=?, "
                    + "workplace=?, bio=?, portfolio=?, linkedin=?, password=?,locked=?,blockstatus=? WHERE id=?";
            ...
            ps.setString(9, expert_domain);
            ps.setString(10, experience);
            ps.setString(11, workplace);
            ps.setString(12, bio);
            ps.setString(13, portfolio);
            ps.setString(14, linkedin);
            ps.setString(15, passwordToUse);

            // Set the locked status based on whether password was changed
            if (passwordChanged) {
                ps.setString(16, "yes"); // Lock the account
                ps.setString(17, "blocked");
            } else {
                ps.setString(16, "no"); // Keep account unlocked
...
//            out.println("<a href='EmailForm.jsp'>Send another email</a>");
                ...
                out.println("<script type='text/javascript'>");
                out.println("alert('Specialist profile updated successfully!');");
                out.println("location='Admin_interface/specialist_details.jsp?message=updated';");
                out.println("</script>");
            } else {
